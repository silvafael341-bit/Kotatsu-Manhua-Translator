...
class ManhuarmTranslator(
    ...
    suspend fun translatePage(
        ...
        val recognitions = listOf(
            runCatching {
                latinRecognizer.process(image).awaitTask()
            }.getOrNull(),

            runCatching {
                chineseRecognizer.process(image).awaitTask()
            }.getOrNull(),

            runCatching {
                japaneseRecognizer.process(image).awaitTask()
            }.getOrNull(),

            runCatching {
                koreanRecognizer.process(image).awaitTask()
            }.getOrNull(),
        ).filterNotNull()

        val best = recognitions.maxByOrNull { score(it) }
            ?: return@withContext emptyList()
        ...
        best.textBlocks
            ...
            .mapNotNull { line ->
                ...
                if (text.length < 2) {
                    ...
                    line.boundingBox?.let { Rect(it) to text }
                }
            }
            ...
            .mapNotNull { (rect, text) ->
                runCatching {
                    val translated =
                        googleTranslate.translate(text, "auto", targetLanguage).trim()

                    if (translated.isBlank()) {
                        null
                    } else {
                        TranslationRegion(rect, text, translated)
                    }
                }.getOrNull()
            }
    }
