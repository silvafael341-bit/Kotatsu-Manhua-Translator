...
abstract class BasePagerReaderFragment : BaseReaderFragment<FragmentReaderPagerBinding>(),
    ...
    override fun onViewBindingCreated(binding: FragmentReaderPagerBinding, savedInstanceState: Bundle?) {
        ...
        with(binding.pager) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) recyclerView?.defaultFocusHighlightEnabled = false
        ...
        viewModel.pageAnimation.observe(viewLifecycleOwner) {
            ...
            if (transformer == null) binding.pager.recyclerView?.children?.forEach { view -> view.resetTransformations() }
        }
    }
    ...
    override suspend fun onPagesChanged(pages: List<ReaderPage>, pendingState: ReaderState?) = coroutineScope {
        ...
        if (pendingState != null) {
            val position = pages.indexOfFirst { it.chapterId == pendingState.chapterId && it.index == pendingState.page }
            items.join()
            if (position != -1) {
    ...
    override fun getCurrentState(): ReaderState? = viewBinding?.run {
        ...
        ReaderState(chapterId = page.chapterId, page = page.index, scroll = 0)
    }

    protected open fun onWheelScroll(axisValue: Float) { switchPageBy(-axisValue.sign.toInt()) }
    protected open fun onCreateAdvancedTransformer(): PageTransformer = PageAnimTransformer()
    protected open fun onInitPager(pager: ViewPager2) { pager.offscreenPageLimit = 2 }
    protected open fun notifyPageChanged(page: Int) { viewModel.onCurrentPageChanged(page, page) }

    companion object { const val SMOOTH_SCROLL_LIMIT = 3 }
}
